...
void threshold(double tsl1401cl_data[], int ThresholdData[], int tsl1401cl_size, double threshold)
{
    for (int i = 0; i < TSL1401CL_SIZE; i++)
    {
        ...
        {
            ThresholdData[i] = 180;//라인이 검출되는 곳은 180 출력
        }
        else
        {
            ThresholdData[i] = 0;// 아닌곳은 0출력
        }
    }
}
...
void tsl1401cl_Callback(const std_msgs::Float32MultiArray::ConstPtr &msg)
{
    ...
    for (int i = 0; i < TSL1401CL_SIZE; i++)
    {
        tsl1401cl_data[i] = msg->data[i];// 카메라 데이터를 받아서 tsl1401cl_data 배열에 저장
    }
    threshold(tsl1401cl_data, LineSensor_threshold_Data, TSL1401CL_SIZE, THRESHOLD);// 위에서 받은 tsl1401cl_data를 이용해 LineSensor_threshold_Data를 만듬
}
...
void lane_control(geometry_msgs::Twist &cmd_vel)
{
    ...
    Steering_Angle = Kp * error + Kd * error_d + Ki * 0;  // Ki는 일단 0으로 설정
